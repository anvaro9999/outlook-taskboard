...
tbApp.controller('taskboardController', function ($scope, $filter) {
    ...
    $scope.init = function () {
		...
		if ($scope.config.INFOCUS_FOLDER.ACTIVE) $scope.numfolders++;
        ...
        $scope.sortableOptions = {
            ...
            stop: function (e, ui) {
                ...
                if (ui.item.sortable.droptarget) { // check if it is dropped on a valid target
                    if (($scope.config.INFOCUS_FOLDER.LIMIT !== 0 && e.target.id !== 'infocusList' && ui.item.sortable.droptarget.attr('id') === 'infocusList' && $scope.infocusTasks.length > $scope.config.INFOCUS_FOLDER.LIMIT) ||
					($scope.config.INPROGRESS_FOLDER.LIMIT !== 0 && e.target.id !== 'inprogressList' && ui.item.sortable.droptarget.attr('id') === 'inprogressList' && $scope.inprogressTasks.length > $scope.config.INPROGRESS_FOLDER.LIMIT) ||
                    ...
                    switch (ui.item.sortable.droptarget[0].id) {
                        ...
                        case 'inprogressList':
                            ...
                            break;
						case 'infocusList':
                            var tasksfolder = getOutlookFolder($scope.config.INFOCUS_FOLDER.NAME);
                            var newstatus = $scope.config.STATUS.IN_FOCUS.VALUE;
                            break;
                        case 'waitingList':
        ...
        for (i = 0; i < categories.length; i++) {
            ...
            if (categories[i].length > 0) {
                if ($scope.useCategoryColors) {
                    catStyles[i] = {
                        //avr: label: categories[i], style: { "background-color": getColor(categories[i]), color: getContrastYIQ(getColor(categories[i])) }
                        label: categories[i], style: { "border-left-color": getColor(categories[i]) }
                    }
                }
                else {
                    catStyles[i] = {
                        label: categories[i], style: { "border-left-color": "black" }
                    };
                }
            }
        }
    ...
    $scope.initTasks = function () {
		...
		$scope.infocusTasks = getTasksFromOutlook($scope.config.INFOCUS_FOLDER.NAME, $scope.config.INFOCUS_FOLDER.RESTRICT, $scope.config.INFOCUS_FOLDER.SORT, $scope.config.STATUS.IN_FOCUS.VALUE);
		...
		$scope.filteredInfocusTasks = $scope.infocusTasks;
    ...
    $scope.applyFilters = function () {
        if ($scope.search.length > 0) {
			...
			$scope.filteredInfocusTasks = $filter('filter')($scope.infocusTasks, $scope.search);
        ...
        if ($scope.config.PRIVACY_FILTER) {
			...
			$scope.filteredInfocusTasks = $filter('filter')($scope.filteredInfocusTasks, function (task) { return task.sensitivity == sensitivityFilter });
    ...
    $scope.createReport = function () {
		...
		// INFOCUS ITEMS
        if ($scope.config.INFOCUS_FOLDER.REPORT.DISPLAY) {
            var tasks = getOutlookFolder($scope.config.INFOCUS_FOLDER.NAME).Items.Restrict("[Status] = 1 And Not ([Sensitivity] = 2)");
            tasks.Sort("[Importance][Status]", true);
            mailBody += "<h3>" + $scope.config.INFOCUS_FOLDER.TITLE + "</h3>";
            mailBody += "<ul>";
            var count = tasks.Count;
            for (i = 1; i <= count; i++) {
                mailBody += "<li>"
                if (tasks(i).Categories !== "") { mailBody += "[" + tasks(i).Categories + "] "; }
                mailBody += "<strong>" + tasks(i).Subject + "</strong>" + " - <i>" + taskStatus(tasks(i).Status) + "</i>";
                if ($scope.config.INFOCUS_FOLDER.DISPLAY_PROPERTIES.TOTALWORK) { mailBody += " - " + tasks(i).TotalWork + " mn "; }
                if (tasks(i).Importance == 2) { mailBody += "<font color=red> [H]</font>"; }
                if (tasks(i).Importance == 0) { mailBody += "<font color=gray> [L]</font>"; }
                var dueDate = new Date(tasks(i).DueDate);
                if (moment(dueDate).isValid && moment(dueDate).year() != 4501) { mailBody += " [Due: " + moment(dueDate).format("DD-MMM") + "]"; }
    ...
    var taskStatus = function (status) {
		...
		if (status == $scope.config.STATUS.IN_FOCUS.VALUE) { str = $scope.config.STATUS.IN_FOCUS.TEXT; }
	...
	$scope.getPanelStyle = function (categories) {
        if ($scope.useCategoryColorFooters) {
            if ((categories !== '') && $scope.useCategoryColors) {
                // Copy category style
                if (categories.length == 1) {
                ...
                // Make multi-category tasks light gray
                    ...
                    var lightGray = '#dfdfdf';
                    //avr: return { "background-color": lightGray, color: getContrastYIQ(lightGray) };
					return { "border-left-color": "lightGray" };
                }           
            }
        }
        return;
